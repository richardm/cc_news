...
- **Keyword expansion and entity discovery.** A data analyst retrieves articles matching initial keywords, then identifies additional keywords, entities, or tags from those articles to broaden the search. The analyst maintains a working keyword/phrase list and an ignore list (stop words, low-value terms) throughout the session, iteratively refining both as they explore. Article previews highlight matches against the keyword list, enabling quick visual scanning. The CLI provides article retrieval and text extraction; entity recognition may live in consuming projects.
